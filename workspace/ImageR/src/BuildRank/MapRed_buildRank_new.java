...
import MyAPI.imagR.ImageIndexInfo;
...
import MyCustomedHaoop.KeyClass.Key_QID_reRankFlagGroupID;
...
public class MapRed_buildRank_new extends Configured implements Tool{
...
//		prepareData();
	...
	public static void runHadoop(String[] args) throws Exception {
		oriArgs=args;
		int ret = ToolRunner.run(new MapRed_buildRank_new(), args);
		System.exit(ret);
	}
...
//		}
	...
	public int run(String[] args) throws Exception {//this args is already parsed by ToolRunner, so not all the oriArgs
		...
		boolean includeNoRerank=true;//this is oriRank, no rerank
				...
				for (int rer_i = 0; rer_i < reRTopRInfo.getReRankNum(); rer_i++) {
					...
					for (int rerHEPara_i = 0; rerHEPara_i < reRankHEParas.size(); rerHEPara_i++) {
						...
						if(isRunThisLoop){//guaratee HMW-delta used in iniR_scheme is the same with reRankHEPara
							...
							currentLoopLabel=queryloopLabel+iniRSchemesloopLabel+rerankLoopLabel+rerHDrLoopLabel+"_F"+"-"+rerankFlags.size();
							...
							Conf_ImageR.addDistriCache_rerankFlagsData(rankFlagsData, conf, cacheFilePaths); //rankFlagsData path with symLink
	...
	public static class Reducer_SearchTVector_getHMScore extends Reducer<IntWritable,IntArr_HESig_ShortArr_Arr,IntWritable,VW_DID_Score_Arr>  {
		...
		public void reduce(IntWritable VW, Iterable<IntArr_HESig_ShortArr_Arr> QueryNames_feats, Context context) throws IOException, InterruptedException {
        	...
        	General.dispInfo_ifNeed(disp, "", "one reduce finished! total query number for this vw: "+queryNum_thisVW+", queryFeatNum_thisVW:"+queryFeatNum_thisVW+", queryNum_existMatch:"+queryNum_existMatch
        			+", stat QMachDocNum_QFeatNum:"+stat.getFullStatistics("0"));
        	disp=false;
        	
        	vws_queryNums.append(vw+"_"+queryNum_thisVW+"_"+queryFeatNum_thisVW+"_"+queryNum_existMatch+", "+stat.getFullStatistics("0")+"\n");
	...
	public static class Reducer_buildRank_final_ParaliseQuery_saveScore extends Reducer<Key_QID_reRankFlagGroupID, DocAllMatchFeats, Key_RankFlagID_QID, QID_IntList_FloatList>  {
		...
		ArrayList<String> rerankFlags;
		private ImageIndexInfo indexInfo;
...
//		private boolean debugFail;
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			System.out.println("read rerankFlags finished! tot-rerankFlags num:"+rerankFlags.size());
			//********** setup scoreDoc, makeRank***************
			//-setup_indexInfo 
			indexInfo=new ImageIndexInfo(conf_ImageR);
			//-setup_makeRank
...
//			debugFail=false;
		...
		@Override
		public void reduce(Key_QID_reRankFlagGroupID key, Iterable<DocAllMatchFeats> values, Context context) throws IOException, InterruptedException {
			/**
			...
			int QID=key.obj_1.get();
			//set reRankFlagIDs for this reRankFlagGroupID
			int[] reRankFlagIDs=General.makeRange(new int[]);
			//get query feat
			SURFpoint_ShortArr queryFeats=new SURFpoint_ShortArr();
			if(queryFeatReader.get(new IntWritable(QID), queryFeats)==null){
				throw new InterruptedException("err! no feat for query:"+QID);
			}
			//get query size
			int[] querySize=QuerySize_HashMap.get(QID);
			int queryMaxSize=Math.max(querySize[0], querySize[1]);
			//run
			int docNum=0;
			for (DocAllMatchFeats docAllMatchFeats : values) {
				float[] docScores=scoreDoc.scoreOneDoc(docAllMatchFeats, queryFeats.getArr(), QID, queryMaxSize, null, null, null, disp&&(docNum%dispInter_D==0));
			...
			addTailFromIniRank(new IntWritable(QID), returnRank, TailFromIniRank);
			//output
			context.write(new Key_RankFlagID_QID(conf_ImageR.sd_rankFlagInd,QID), new QID_IntList_FloatList(QID,returnRank));
			General.Assert(processedQuery.add(QID), "err! duplicated queryID:"+QID);
			//done
...
//			}
	...
	public static class Reducer_buildRank_final_ParaliseQuery_saveDocMatches extends Reducer<Key_QID_reRankFlagGroupID, DocAllMatchFeats, Key_RankFlagID_QID, QID_PhotoPointsLoc_DID_Score_ImageRegionMatch_ShortArr_Arr>  {
