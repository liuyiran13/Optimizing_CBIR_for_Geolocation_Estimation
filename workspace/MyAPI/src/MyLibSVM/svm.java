...
//		svm.info("epsilon = "+(-si.r)+"\n");
	...
	public static class decision_function implements java.io.Serializable
	{
		public double[] alpha;
		public double rho;	
	};
...
//		svm.info("nSV = "+nSV+", nBSV = "+nBSV+"\n");
	...
	public static void sigmoid_train(int l, double[] dec_values, double[] labels, double[] probAB) {//label should be +1 or -1
		double A, B;
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB, int nr_fold)
	{
		int i;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			int j,k;
			svm_problem subprob = new svm_problem(); //save train-data
			...
			k=0;
			for(j=0;j<begin;j++)//save train data in prob, data befor test start
			{
				subprob.x[k] = prob.x[perm[j]];
				subprob.y[k] = prob.y[perm[j]];
				++k;
			}
			for(j=end;j<prob.l;j++)//save train data in prob, data after test end
			{
				subprob.x[k] = prob.x[perm[j]];
				subprob.y[k] = prob.y[perm[j]];
				++k;
			}
			int p_count=0,n_count=0; //caculate train-data, positive and negtive number
			for(j=0;j<k;j++)
				if(subprob.y[j]>0)
					p_count++;
				else
			...
			if(p_count==0 && n_count==0)//train data is empty
				for(j=begin;j<end;j++)
					dec_values[perm[j]] = 0;
			else if(p_count > 0 && n_count == 0) //train data is all positive
				for(j=begin;j<end;j++)
					dec_values[perm[j]] = 1;
			else if(p_count == 0 && n_count > 0) //train data is all negative
				for(j=begin;j<end;j++)
					dec_values[perm[j]] = -1;
			else //train data have both positive and negative
			{
				...
				svm_model submodel = svm_train(subprob,subparam,-1);
				for(j=begin;j<end;j++)
				{
	...
	private static double svm_svr_probability(svm_problem prob, svm_parameter param)
	{
		...
		svm_cross_validation(prob,newparam,nr_fold,ymv,-1);
		for(i=0;i<prob.l;i++)
		{
	...
	public static void svm_group_classes(svm_problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm)
	{
		int l = prob.l;
		int max_nr_class = 16;
		int nr_class = 0;
	...
	// Interface functions
	//
	public static svm_model svm_train(svm_problem prob, svm_parameter param,int nr_fold){ //nr_fold is used when train probability model for n-fold Cross-validation decision values for probability estimates, probAB !
		...
		else
		{
			...
			boolean[] nonzero = new boolean[l]; //nonZero is to mark which train sample in the support vector, is combined from all sub-1vs1-models!!
			for(i=0;i<l;i++)
				nonzero[i] = false;
			...
			for(i=0;i<nr_class;i++)
				for(int j=i+1;j<nr_class;j++)
				{
					...
					if(param.probability == 1)
					{
						...
						svm_binary_svc_probability(sub_prob,param,weighted_C[i],weighted_C[j],probAB,nr_fold);
						probA[p]=probAB[0];
						probB[p]=probAB[1];
					}
					//train one decision function
					f[p] = svm_train_one(sub_prob,param,weighted_C[i],weighted_C[j]);
					for(k=0;k<ci;k++)
						if(!nonzero[si+k] && Math.abs(f[p].alpha[k]) > 0) //updata nonZero
							nonzero[si+k] = true;
					for(k=0;k<cj;k++)
...
//			svm.info("Total nSV = "+nnz+"\n");
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target, int nr_fold_proModel)
	{
		int i;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			svm_model submodel = svm_train(subprob,param,nr_fold_proModel);
			if(param.probability==1 &&
...
//			System.out.println(dec_values.length+" dec_values, time:"+General.dispTime(System.currentTimeMillis()-tempTime, "ms"));
