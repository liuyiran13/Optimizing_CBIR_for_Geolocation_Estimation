...
public class HistMultiD_Sparse_generalBin_forFloat <M extends Object> {//multi-dimensional hist, as it is Sparse, use hashMap instead of muti-dim array
	/*
	...
	protected HashMap<Integer, Integer> hist;
	protected HashMap<Integer, Integer> hist_masterBin_forOneVoteForTwoBin; //for oneVoteForTwoBin, it may generate two bin that is max, in this case, we use materBinVote to decide which bin to choose
	protected int sampleNum;
	protected boolean saveSample;
	protected HashMap<Integer, ArrayList<M>> samplesInBins;
	protected int[] MaxBin_ind_val;
	
	
	public HistMultiD_Sparse_generalBin_forFloat(boolean saveSample, boolean oneVoteForTwoBin){
	...
	public void iniHist() {
		...
		if (oneVoteForTwoBin) {
			this.hist_masterBin_forOneVoteForTwoBin=new HashMap<Integer, Integer>();
		}
		if (saveSample) {
			this.samplesInBins=new HashMap<Integer, ArrayList<M>>();
		}
		sampleNum=0;
		MaxBin_ind_val=null;
	}
	...
	protected void addOneSample_assignTwoBins(float[] sampleV, M sample) {
		...
		General.updateMap(hist_masterBin_forOneVoteForTwoBin, binIndInOneDim, 1);
		//
		sampleNum++;
	}
	...
	public int[] getMaxBin_ind_val() {
		if (MaxBin_ind_val==null) {
			LinkedList<Entry<Integer, Integer>> maxEntry= General.getMinMax_entry(hist).get(1);
			Entry<Integer, Integer> finalSel=null;
			if (oneVoteForTwoBin && maxEntry.size()>1) {
				int max_masterValue=Integer.MIN_VALUE; 
				for (Entry<Integer, Integer> one : maxEntry) {
					Integer masterValue=hist_masterBin_forOneVoteForTwoBin.get(one.getKey());
					if (masterValue!=null && max_masterValue<masterValue) {
						max_masterValue=masterValue;
						finalSel=one;
					}
				}
				if (finalSel ==null ) {//all bins in maxEntry are not master bins, are voted by the nearby bin
					finalSel=maxEntry.getFirst();
				}
			}else {
				finalSel=maxEntry.getFirst();
			}
			MaxBin_ind_val=new int[]{finalSel.getKey(), finalSel.getValue()};
		}
	...
	public float[][] getOneBinRang(int binInd) {
		...
		for (int i = 0; i < bins_eachDim.length; i++) {
			bins_eachDim[i]=getOneBinRang_eachDim(binInds_eachDim[i], bins[i], bins_middle[i]);
		}
		return bins_eachDim;
	}
	
	public float[] getOneBinRang_eachDim(int binInd_eachDim, float[] thisDimBin, float[] thisDimMiddleBin) {
		if (oneVoteForTwoBin) {//return two nearby middle bin's rang
			if (thisDimMiddleBin.length==2) {//binInd_eachDim can only be 0 or 1 or 2
				if (binInd_eachDim==0) {
					return new float[]{Integer.MIN_VALUE, thisDimMiddleBin[1]};
				}else if (binInd_eachDim==2) {
					return new float[]{thisDimMiddleBin[1], Float.MAX_VALUE};
				}else {
					return new float[]{thisDimBin[0], thisDimBin[1]};
				}
			}else if (binInd_eachDim<2) {
				return new float[]{Integer.MIN_VALUE, thisDimMiddleBin[binInd_eachDim+1]};
			}else if (binInd_eachDim>thisDimBin.length-2) {
				return new float[]{thisDimMiddleBin[binInd_eachDim-1], Float.MAX_VALUE};
			}else {
				return new float[]{thisDimMiddleBin[binInd_eachDim-1], thisDimMiddleBin[binInd_eachDim+1]};
			}
		}else {
			if (binInd_eachDim==0) {
	...
	public float[] getNormlisedHist(){
		float[] normHist=new float[totBinNum]; int totVoteNum=sampleNum*(oneVoteForTwoBin?2*eachDimSize.length:1);
		for (Entry<Integer, Integer> oneBin : hist.entrySet()) {
			normHist[oneBin.getKey()]=(float)oneBin.getValue()/totVoteNum;
		}
		return normHist;
	}
	...
	public String makeRes(String floatValueFormat, boolean show_samplesInBins, boolean show_hist) {
		if (show_samplesInBins) {
			return "sampleNum:"+sampleNum+", tot-"+eachDimSize.length+" dim, eachDimSize:"+General.IntArrToString(eachDimSize, "_")+(show_hist?", hist on this bin:"+hist:"")+
