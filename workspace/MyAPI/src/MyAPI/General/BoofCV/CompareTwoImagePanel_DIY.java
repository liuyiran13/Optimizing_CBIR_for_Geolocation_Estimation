...
public abstract class CompareTwoImagePanel_DIY extends JPanel implements MouseListener , MouseMotionListener{
	...
	protected List<SURFpoint> leftPts,rightPts;
	...
	public CompareTwoImagePanel_DIY(int borderSize , boolean canSelectRegion) {
		this(borderSize, canSelectRegion, 1);
	}
	
	public CompareTwoImagePanel_DIY(int borderSize , boolean canSelectRegion, int pointEnlargeFactor) {
		this.borderSize = borderSize;
		this.selectRegion = canSelectRegion;
		/**make point Enlarged, beacuse some image is quite large, so it needs to be shriked, 
		 * and each point is then shriked to very small size,
		 * pointEnlargeFactor only scale the point's scale, not influent the x, y, orientation
		 */
	...
	public void setLocation( List<SURFpoint> leftPts , List<SURFpoint> rightPts) {
...
//		System.out.println("selected index "+selectedIndex);
	...
	private int findBestPoint(int x, int y,  List<SURFpoint> pts ) {
	...
	private void findPointsInRegion( int x0 , int y0 , int x1 , int y1 , List<SURFpoint> pts )
	{
		selected.clear();
		for( int i = 0; i < pts.size(); i++ ) {
			if( !isValidPoint(i) )
